...
def reset():
    url = 'http://localhost:80/elm/groups/Group01/performer?active=1&sequenceId=25'
    requests.post(url)

def on_click():
    # autopy.key.tap(autopy.key.Code.DOWN_ARROW)
    autopy.mouse.click()
    url = 'http://localhost:80/elm/groups/Group01/performer?active=1&sequenceId=29'
    requests.post(url)
    print("click")
...
def main():
    ...
    cv2.createTrackbar('X','Archand',0, offset * 2, empty_callback)
    cv2.createTrackbar('Y','Archand',0, offset * 2, empty_callback)
    ...
    while True:
        ...
        x_offset = cv2.getTrackbarPos('X','Archand') - offset
        y_offset = cv2.getTrackbarPos('Y','Archand') - offset

        leftBound = round(w/2 - frameW) + x_offset
        rightBound = round(w / 2 + frameW) + x_offset
        topBound = round(h/2-frameH) + y_offset
        botBound = round(h/2 + frameH) + y_offset
        
        cv2.rectangle(img, (leftBound, topBound) , (rightBound, botBound), (255, 0, 255), 2)

        if len(lmList) != 0:
            x1 = lmList[detector.fingerTipIndices[1]][1]
            y1 = lmList[detector.fingerTipIndices[1]][2]
